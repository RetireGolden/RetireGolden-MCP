...
function expectLine(subject, dottedPath, value) {
  ...
  // A non-zero value this small is float residue from the solver, not a modeled
  // amount, and tests/goldens.test.ts deliberately refuses to pin it: it
  // asserts toBeCloseTo(0, 6). EMIT that matcher rather than a literal — a
  // printed `.toBe(6.4e-12)` is paste-ready by construction, and pasting it
  // would silently freeze a bit pattern the suite chose not to freeze. A
  // comment asking the reader not to paste what was just printed is not a
  // safeguard.
  if (typeof value === 'number' && value !== 0 && Math.abs(value) < 1e-6) {
    return `    ${target}.toBeCloseTo(0, 6)  // solver residue (${String(value)}), deliberately not pinned`
  }
  return `    ${target}.toBe(${String(value)})`
}
...
function printSet(modules, set) {
  ...
  const lines = []
  // The `describe`/`it` scaffolding is printed only so each number arrives with
  // the assertion it belongs to. It is NOT a drop-in replacement for the
  // committed block: `it` titles here are generic where the test's are
  // descriptive, and the test carries assertions this generator does not model
  // (the SINGLE new-defaults block's build-caveat test, for one). Replacing a
  // committed `describe` wholesale with this output deletes that coverage.
  lines.push(`// Numbers only — copy individual expect(...) lines into the committed block.`)
  lines.push(`// Do NOT replace the committed describe with this scaffolding.`)
...
async function main() {
  ...
  console.log('// Generated by scripts/gen-goldens.mjs — copy INDIVIDUAL expect(...) lines')
  console.log('// into tests/goldens.test.ts, deliberately, never to make a red test green.')
  console.log('// The describe/it scaffolding below exists to place each number; it is not')
  console.log('// a replacement for a committed block. The test adds assertions this')
  console.log('// generator does not model (build caveats, the engine-default passthrough')
  console.log('// suite) and uses more descriptive it() titles — pasting a whole block over')
  console.log('// a committed one drops that coverage.')
  console.log('')
