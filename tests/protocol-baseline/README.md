...
- Each successful tool call stores both a `payloadHash` (first text block parsed as JSON) and an `envelopeHash` over the full `CallToolResult` shape: `isError`, every content block (text blocks as parsed canonical JSON, other blocks by type and sorted property keys), and optional `structuredContent` / `_meta` when present. Reordering, adding, or removing content blocks therefore fails the baseline even when the first text payload is unchanged.
